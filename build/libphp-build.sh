...
# static-musl (no distribution ships a static libphp). The binary must refuse
...
# -include: fpm.c calls zend_signal_use_persistent_handlers(), which no
# distribution header declares, and an implicit declaration is an error on
# current compilers. The prototype has to reach every translation unit.
