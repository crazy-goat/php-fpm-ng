...
arguing for survives: the status and metrics pages are now `operator.status_path` and
`operator.metrics_path` on the pool being watched, answered by the operator endpoint
