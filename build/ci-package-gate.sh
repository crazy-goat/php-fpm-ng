...
deb)
    IMAGE=ubuntu:26.04
    if [ "$TLS_PACKAGE" = 1 ]; then EXPECT_PASS=60; EXPECT_SKIP=36
    else EXPECT_PASS=55; EXPECT_SKIP=41; fi
...
apk)
    IMAGE=alpine:edge
    if [ "$TLS_PACKAGE" = 1 ]; then EXPECT_PASS=58; EXPECT_SKIP=38
    else EXPECT_PASS=53; EXPECT_SKIP=43; fi
