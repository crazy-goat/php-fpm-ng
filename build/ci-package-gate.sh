...
deb)
    IMAGE=ubuntu:26.04
    if [ "$TLS_PACKAGE" = 1 ]; then EXPECT_PASS=77; EXPECT_SKIP=36
    else EXPECT_PASS=72; EXPECT_SKIP=41; fi
...
apk)
    IMAGE=alpine:edge
    if [ "$TLS_PACKAGE" = 1 ]; then EXPECT_PASS=75; EXPECT_SKIP=38
    else EXPECT_PASS=70; EXPECT_SKIP=43; fi
