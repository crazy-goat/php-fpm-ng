...
deb)
    IMAGE=ubuntu:26.04
    if [ "$TLS_PACKAGE" = 1 ]; then EXPECT_PASS=142; EXPECT_SKIP=7
    else EXPECT_PASS=135; EXPECT_SKIP=14; fi
...
apk)
    IMAGE=alpine:edge
    if [ "$TLS_PACKAGE" = 1 ]; then EXPECT_PASS=139; EXPECT_SKIP=10
    else EXPECT_PASS=133; EXPECT_SKIP=16; fi
