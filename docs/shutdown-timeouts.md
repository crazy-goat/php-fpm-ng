...
|---|---|---|---|
| `fastcgi`, `http` (+ `fiber`/`async` executors) | `request_terminate_timeout` (per request; default 0 = none) | `process_control_timeout` (master escalation after `SIGTERM` to the master) | `request_terminate_timeout` when set |
